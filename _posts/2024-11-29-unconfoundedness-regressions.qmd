...
&bull; $$ATT(X) := \E[Y(1) - Y(0) | X]$$ - the $$ATT$$ for a given value of the covariates $$X$$.

&bull; $$\L(D|X)$$ - a (possibly misspecified) linear probability model for the propensity score.  The notation stands for the linear projection of $$D$$ on $$X$$.

&bull; $\L_0(Y|X)$ - a (possibly misspecified) linear model for $$\E[Y|X,D=0]$$.
:::
